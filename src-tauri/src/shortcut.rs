...
pub use global_hotkey::{
    GlobalHotKeyEvent as ShortcutEvent, HotKeyState as ShortcutState, hotkey::HotKey as Shortcut,
};
use global_hotkey::{
    GlobalHotKeyEvent,
...
impl<R: Runtime> GlobalShortcut<R> {
        ...
        T::Error: std::error::Error,
    {
        ...
        for s in mapped_shortcuts {
            self.shortcuts.remove(&s.id());
        }

        Ok(())
    }

    async fn get_hotkeys(&self) -> Vec<Shortcut> {
        let mut hotkeys = Vec::new();
        self.shortcuts
            .scan_async(|_, s| {
                hotkeys.push(s.shortcut);
            })
            .await;
        hotkeys
    }
        ...
        S::Error: std::error::Error,
    {
        if let Ok(shortcut) = try_into_shortcut(shortcut) {
            self.shortcuts.contains(&shortcut.id())
        } else {
            false
        }
    }
    ...
    pub fn build(self) -> TauriPlugin<R> {
        ...
        PluginBuilder::new("global-shortcut")
            .setup(move |app, _api| {
                ...
                let store = HashMap::<HotKeyId, RegisteredShortcut<R>>::new();
                for shortcut in shortcuts {
                ...
                let shortcuts = Arc::new(store);
                let shortcuts_ = shortcuts.clone();

                let app_handle = app.clone();
                GlobalHotKeyEvent::set_event_handler(Some(move |e: GlobalHotKeyEvent| {
                    if let Some(shortcut) = shortcuts_.get(&e.id) {
...
error_set! {
    Error = {
        ...
        GlobalHotkey(global_hotkey::Error),
        ParseHotkey(global_hotkey::hotkey::HotKeyParseError),
        Tauri(tauri::Error),
